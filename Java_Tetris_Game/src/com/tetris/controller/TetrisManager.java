...
//		BOARD_ROW_SIZE 배열을 반복문을 돌려 mBoard[i]를 BoardType.EMPTY로 초기화 시킴
//		Arrays.fill(Object obj, param) : obj 배열을 param으로 전체 초기화하는 메소드
...
//	아래에 쌓인 블럭들 중 지워진 블럭들을 검색하는 메소드
	private void searchLineIndexesToDelete (ArrayList<Integer> indexes) {
//		ArrayList clear() : ArrayList 안의 모든 값이 삭제되는 메소드
		indexes.clear();
...
//	아래에 쌓인 블럭 지우는 메소드
	private void deleteLines (ArrayList<Integer> indexes) {
		...
		for (int i = 0; i < BOARD_ROW_SIZE; i++) {
//			Arrays.fill(Objecct obj, param) : obj 배열을 param이라는 변수로 치환하는 메소드 
